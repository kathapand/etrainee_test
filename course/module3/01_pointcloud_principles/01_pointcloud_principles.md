...
In this theme, you will learn about 3D/4D point cloud data, fundamental operations and concepts of change analysis:

* **[Introduction to Geographic 3D/4D Point Clouds](#introduction-to-geographic-3d4d-point-clouds)**
  * [Importance of 3D/4D geographic point clouds](#importance-of-3d4d-geographic-point-clouds)
  * [Multitemporal and 4D Point Clouds](#multitemporal-and-4d-point-clouds)
  * [Concepts of Change in Multitemporal and 4D Point Clouds](#concepts-of-change-in-multitemporal-and-4d-point-clouds)
* **[Point Cloud Model and Fundamental Operations](#point-cloud-model-and-fundamental-operations)**
  * [Point Cloud Data Model](#point-cloud-data-model)
  * [Multisource and Multitemporal Data Fustion](#multisource-and-multitemporal-data-fusion)
  * [Neighborhood Operations and Neighborhood-Based Attributes](#neighborhood-operations-and-neighborhood-based-attributes)
  * [Transformation of 3D point cloud data](#transformation-of-3d-point-cloud-data)
* **[Data Management of 4D Point Clouds](#data-management-of-big-4d-point-clouds)**
  * [Data formats](#data-formats)
  * [Metadata and Supplementary Datasets](#metadata-and-supplementary-datasets) 

[//]: <> ([TODO SZ] Check if table of contents corresponds to actual sections and headings)
...
<i>Examples of different domains supported by change analysis of surface or structure dynamics in 3D/4D point clouds.</i>
</center>
...
<i>Sources of laser scanning data covering different ranges of typical spatial and temporal resolutions.</i>
</center>
...
[//]: <> ([TODO SZ] All references in image contained in the bibliography?)
...
<i>Schematic concept of virtual laser scanning with HELIOS++, showcasing platforms (tripod, car, UAV, airplane) and 3D object models (mesh, point cloud, voxels, raster) composing a scene. Source: [Winiwarter et al. (2022b)](#references).</i>
</center>

In this module, you will learn to create VLS point clouds with HELIOS++ for your own analyses ([exercise in Theme 2](../02_pointcloud_programming_python/exercise/m3_theme2_exercise1.md)). For alternative implementations of LiDAR simulation and their specific purposes and differences, see the detailed literature review in [Winiwarter et al., 2022b](#references)
...
<center>
<img src="media/m3_theme1_pointcloud_model_list.png" alt="pointcloud_model" title="Point cloud model in ascii table" width="400">

<i>Visual and tabular representation of a 3D point cloud with three coordinates (XYZ) and attributes classification (ID), LiDAR backscatter (Intensity), and true color (RGB). Source: Ghamisi et al. (2019).</i>
</center>

[//]: <> ([TODO] Harmonize / To what extent explained in module 1? Link or expand.)
...
<center>
<img src="media/m3_theme1_pointcloud_model_fusion.png" alt="pointcloud_model" title="Point cloud model in ascii table" width="400">

<i>Fusion of photographic data with a LiDAR point cloud by assigning color values as RGB attributes to the point cloud. Source: Ghamisi et al. (2019).</i>
</center>
...
<i>Comparison of terrain representation as point cloud and raster.</i>
</center>
...
<center>
<img src="media/m3_theme1_terrain_surface_objects.png" alt="pc_vs_raster" title="Representation of terrain, surface and object structure in point cloud data" width="800">

<i>Representation of terrain, surface and object structure in 3D point clouds.</i>
</center>

In this module, you will learn about change analysis methods using the full 3D data provided by point clouds, as well as raster-based methods when deriving rasterized terrain models from point cloud data (especially in [Themes 2](../02_pointcloud_programming_python/02_pointcloud_programming_python.ipynb) and [3](../03_3d_change_analysis/03_3d_change_analysis.md)). If you would like to get deeper into concepts of terrain interpolation from 3D point clouds, take a look at [Pfeifer & Mandlburger, 2008](#references).
...
<i>Cylindrical point cloud neighborhood of a 3D point (black) with a fixed 2D distance of radius r in a schematic view from the side (left) and above (right).</i>
</center>
...
<i>Point cloud neighborhood around a 3D search point (black) with a fixed number of k nearest neighbors in 2D (left) and 3D (right). When considering the 3D Euclidian distance, the point neighborhood may change, because a point may have a vertically large distance compared to another point which is horizontally further away (indicated by the red connection lines).</i>
</center>
...
<center>
<img src="media/kromer_etal_2015_remotesensing-07-13029-g004.png" alt="normalvector" title="Schematic of normal vector of a tangent plane" width="800">

<i>Sphericel spatial neighborhoods (left) and temporal window neighborhood (right) are combined to a spatiotemporal neighborhood (middle) to perform noise filtering in 4D point clouds, with number of neighbors NN, reference point cloud Tref and temporal window Tstep. Source: [Kromer et al., 2015](#references).</i>
</center>

<span style="color:red">[TODO SZ] format all subscripts in figure caption formula style (like in the image itself: Tref etc.)</span>
...
<i>Normal vector of a tangent plane fit to a set of 3D points, which are sampled on a surface in the scene.</i>
</center>
...
<i>Definition of a plane with parameters according to the Hessian normal form (c) Quartl, CC BY-SA 3.0 (https://commons.wikimedia.org/w/index.php?curid=31080804)</i>
</center>
...
<i>Schematic of principal componentes for two sets of 3D point.</i>
</center>
...
<center>
<img src="media/m3_theme1_transformation.png" alt="transformation" title="Schematic of scale-dependent roughness" width="450">

<i>Schematic of scale-dependent roughness where the local plane fit depends on the size of the spatial neighborhood.</i>
</center>
...
In practice, 4D data is often made accessible by appending the observed variables or change attributes to a set of *core points* (more on that in [Themes 3](../03_3d_change_analysis/03_3d_change_analysis.md) and [4](../04_3d_timeseries_analysis/04_3d_timeseries_analysis.md) of this module). By this, for each location in a 3D scene, the full temporal information on each variable is available (corresponding to a 1D time series).
...
<i>Point cloud model with XYZ coordinates and attributes of a variable A for each point in time t of T epochs in the point cloud time series.</i>
</center>
...
<span style="color:red">[TODO SZ] implement quiz questions here</span>

## Exercise 

Made it through the quiz? Then you are ready for the [exercise](#exercise/m3_theme1_exercise1.md), where you will be getting your hands on multisource, multitemporal point cloud data of an active rock glacier.
...
* Eltner, A., Baumgart, P., Domula, A. R., Barkleit, A. & Faust, D. (2017): Scale dependent soil erosion dynamics in a fragile loess landscape. Zeitschrift für Geomorphologie Supplementary Issues, 61(3), pp. 191-206. doi: [10.1127/zfg/2017/0409](10.1127/zfg/2017/0409).
...
* Kromer, R., Abellán, A., Hutchinson, D., Lato, M., Edwards, T., & Jaboyedoff, M. (2015). A 4D Filtering and Calibration Technique for Small-Scale Point Cloud Change Detection with a Terrestrial Laser Scanner. Remote Sensing, 7 (10), pp. 13029-13052. doi: 10.3390/rs71013029. <span style="color:red">[TODO SZ] format.</span>
...
* Chelsea Scott, Minh Phan, Viswanath Nandigam, Christopher Crosby, J Ramon Arrowsmith; Measuring change at Earth’s surface: On-demand vertical and three- dimensional topographic differencing implemented in OpenTopography. Geosphere 2021; doi: https://doi.org/10.1130/GES02259.1 <span style="color:red">[TODO SZ] format.</span>
...
* VanderPlas, J. (2016) <span style="color:red">[TODO SZ] format this book into reference: https://www.oreilly.com/library/view/python-data-science/9781491912126 </span>
...
* Wang, D., Puttonen, E., & Casella, E. (2022). PlantMove: A tool for quantifying motion fields of plant movements from point cloud time series. International Journal of Applied Earth Observation and Geoinformation, 110, pp. 102781. doi: 10.1016/j.jag.2022.102781.
